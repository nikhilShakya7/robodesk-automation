...
| tickets: status filter dropdown filters by status           | @regression | Status dropdown accepts Open, Closed, and Active Conversations; asserts `toHaveValue` + count bounds (product does not filter rows - see Notes) |
| tickets: priority filter dropdown filters by priority       | @regression | Priority dropdown accepts High and All; asserts `toHaveValue` + count bounds (product does not filter rows - see Notes)                      |
